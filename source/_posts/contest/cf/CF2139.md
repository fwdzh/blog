...
恰好所有叶子节点的深度相同时，可以看作是背包问题，我们可以确定哪些值可以达到。如果 $dp_k = 1$，那么答案就为 $d$，否则答案一定是 $d - 1$。当前哪个值的数量 $\ge c_i$，就把当前的 $c_i$ 设为哪个值。由于 $c_i \le c_{i+1}$，深度小于 $d$ 的节点，每个深度的值都可以相同。
...
类似地，可以仅仅使用 bitset 优化即可通过。但我们可以通过一种很奇妙地方法来降低复杂度，由于我们可能会由很多相同的物品，我们可以使用二进制拆分，使得每个大小的物品数量不超过 $\log c_i$，这样总的物品数量大概是 $\sqrt n$ 级别的，可以通过。
